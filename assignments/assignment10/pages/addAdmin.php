...
if(!isset($_SESSION['access']) || (isset($_SESSION['access']) && isset($_SESSION['status']) && ($_SESSION['access'] !== "accessGranted" || $_SESSION['status'] !== "admin"))){
...
function addData($post){
    ...
    /** CHECK FOR A DUPLICATE USERNAME IF FOUND THEN RETURN DUPLICATE OTHERWISE ADD USERNAME AND PASSWORD TO DATABASE */
        ...
        if(count($records) != 0){
            return getForm("<p>There is already someone with that email address</p>", $elementsArr);
        }
        else {
